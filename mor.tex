...
We begin by addressing our first objective -- structure-preserving reduction (SPR) of \cref{eq:chs} using a reduced basis method. To this end, let us consider the following equivalent formulation of \cref{eq:chs}.
\begin{gather}
\left.
...
\right\}
\end{gather}
Here $\fz(t):\mathbb{R} \to \mathbb{R}^{2d}$ and $\flambda$ are the solution of the system, and the structure matrix $\fJ$ is skew-symmetric and non-singular. The smooth function $\tilde{H}(\fz, \flambda) = H(\fz) +\fg^{\top} \flambda$ is the augmented Hamiltonian, and $\fg \in \mathbb{R}^m$ are the constraints. The separable system \eqref{eq:chs} can be recovered from \cref{eq:chs-z} by making these replacements:
$$\fz = \begin{bmatrix}
\fq \\
\fp
\end{bmatrix}, ~ H(\fz) = H(\fq, \fp) +\gamma \fq \cdot \fp, ~ \fJ =
\begin{bmatrix}
\fzero & \fI \\
-\fI & \fzero
\end{bmatrix}.$$
...
respectively, for some $\ff_r(t) \in \mathbb{R}^r$. Assuming $\fV[\top]_r \fW_r = \fI$, it follows that
\begin{align*}
...
Substituting $\fz(t) \approx \fV_r \fz_r(t), ~\nabla_z \tilde{H}(\fV_r \fz_r) \approx \fW_r \nabla_{\fz_r} \tilde{H}_r (\fz_r)$ in \cref{eq:chs-z}, and multiplying the resulting equation with $\fW[\top]_r$ from the left, one gets the reduced system
\begin{gather}
\left.
...
with $\fQ$ being a positive-definite matrix and $h$ being the non-quadratic part of $H$. Let us define $\tilde{h} = h+\fg^{\top} \flambda$, and assume that $\nabla_{\fz} \tilde{h}(\fz(t)) \approx \fU_l \fk_l(t)$ and that $\fV[\top]_r \fQ \fV_r = \fI, ~ \fU[\top]_l \fQ \fU_l = \fI$. Let us further assume the DEIM projection basis $\mathbb{P} = \fU_l (\fE[\top] \fU_l)^{-1} \fE[\top]$ for the hyper-reduction of $\tilde{h}$.
